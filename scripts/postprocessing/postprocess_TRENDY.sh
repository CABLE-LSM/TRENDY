...
#  - August 2024: - replaced gzip compression with cdo compression level
...
#       - soil has 6 layers
#       - soil C has 3 pools
#       - 0.3 of wood is belowground and added to roots (now included in frac_wood_bg)
...
# - double check and, if necessary, rewrite aux/trendy.grid.1deg file (should start i=0 and j=0 at top-left corner)
#    --> note that this will change output dimensions compared to previous versions
# - make sure global variables appear in desired order (e.g. 'commit' after 'source_code')
# - address TODO in line 544!
...
auxdir="${simdir}/aux" 
updir="${simdir}/upload"                          # Upload directory
gridfile="${auxdir}/trendy.grid.1deg"
varfile="${startdir}/variables_TRENDY_v13.csv"    # file that contains all information regarding output variables
...
exps="S0"  # experiments to postprocess
...
#vars="tas pr rsds evapotranspft transpft evapo albedopft snow_depthpft shflxpft rnpft cLitter cProduct cVegpft cSoilpft fLuc soilr rhpft laipft landCoverFrac oceanCoverFrac cLeaf cWood cRoot cCwd tsl msl evspsblveg evspsblsoi tskinpft mslpft theightpft"  # Part 2
...
new_run=0    # 1/0: do/do not delete all previous post-processed variables. 
			 ...
			 # Warning: new_run=1 deletes all existing outputs in the corresponding upload folder!
...
frac_wood_bg=0.3     # fraction of wood pool being belowground (coarse roots)
...
# Output
compression=4         # compression level of output files (0 - 9 (9=highest))
...
if [[ $run_ilamb -eq 1 ]] ; then
   ...
   #export PATH=${PATH}:${HOME}/.local/bin   # using the local installation
   module use /g/data/xp65/public/modules    # using the ACCESS-NRI installation
   module load conda/access-med
   module switch cdo/2.0.5  # needed because conda/access-med seems to load its own cdo (?)
   ...
   if [[ (! -d ${idir}/DATA) && (! -L ${idir}/DATA) ]] ; then   # -d: directory, -L: symbolic link
      echo "ILAMB evaluation data need to be downloaded first (ilamb-fetch) or symlinked to existing data directory"
   fi
fi
...
for exp in ${exps} ; do
	...
	if [[ ! -f cable_output.nc ]] ; then  # only if this hasn't been done before
		...
		cdo settaxis,${startyear}-01-16,12:00:00,1mon cable_tmp.nc cable_tmp1.nc  # ncview will likely display months incorrectly.
	...
	if [[ ! -f casa_output.nc ]] ; then  # only if this hasn't been done before
		## CASA
		if [[ $test_suite -eq 1 ]] ; then
		...
		cdo settaxis,${startyear}-01-16,12:00:00,1mon casa_tmp.nc casa_tmp1.nc  # ncview will likely display months incorrectly.
	...
	## LUC for S3 upwards
	if [[ ("${exp}" == *"S3"*) && (! -f LUC_output.nc) ]] ; then 

		# delete unnecessary LUC variables. 
		# In this case (Aug 2024) the way these variables were chunked slowed everything down substantially.
	    cdo delname,biomass_age_secondary,biomass_age_primary,freq_age_secondary,freq_age_primary \
		    ${simdir}/${exp}/output/cru_out_LUC_${startyear}_1900.nc LUC_tmp11.nc
	    cdo delname,biomass_age_secondary,biomass_age_primary,freq_age_secondary,freq_age_primary \
		    ${simdir}/${exp}/output/cru_out_LUC_1901_${endyear}.nc   LUC_tmp12.nc

		if [[ $test_suite -eq 1 ]] ; then		
            cp ${simdir}/${exp}/output/cru_out_LUC_${startyear}_${endyear}.nc LUC_tmp1.nc
	    else
            cdo -O -s mergetime LUC_tmp11.nc LUC_tmp12.nc LUC_tmp1.nc
	    fi
    ...
    for var in ${vars} ; do
		...
		if [[ "${simvar}" == "calculate" ]] ; then
			...
			cSoilpools)
				...
				;;
			soilr)
				# soil respiration = Crsoil + Rroot (Rroot = Rm_root + Rg_root) at grid cell level
				# where Rg_root = falloc_root * Rg + falloc_root * Clabloss
				# 1) select all required variables (all in gC m-2)
				cdo -O -s selname,Crsoil,patchfrac ${filetype}_output.nc crsoil_tmp.nc                # patch
				cdo -O -s selname,Crmplant ${filetype}_output.nc crm_tmp.nc                           # mplant
				cdo -O -s selname,Crgplant,patchfrac ${filetype}_output.nc crg_tmp.nc                 # patch 
				cdo -O -s selname,Clabloss,patchfrac ${filetype}_output.nc clabloss_tmp.nc            # patch 
				cdo -O -s selname,CallocWood,patchfrac ${filetype}_output.nc calloc_wood_tmp.nc       # patch
				cdo -O -s selname,CallocLeaf,patchfrac ${filetype}_output.nc calloc_leaf_tmp.nc       # patch
				cdo -O -s selname,CallocFineRoot,patchfrac ${filetype}_output.nc calloc_froot_tmp.nc  # patch

				# 2) bring all variables to grid cell level
				ncwa -O -a patch -w patchfrac -v Crsoil crsoil_tmp.nc crsoil_tmp1.nc
				ncatted -O -a cell_methods,Crsoil,d,c, crsoil_tmp1.nc crsoil_tmp.nc   # Rh
                ncks -O -d mplant,${froot} crm_tmp.nc crm_tmp1.nc        # Rm of fine roots
				ncks -O -d mplant,${wood} crm_tmp.nc crm_tmp2.nc        
				cdo  -O -s mulc,${frac_wood_bg} crm_tmp2.nc crm_tmp3.nc  # Rm of coarse roots
				ncwa -O -a patch -w patchfrac -v Crgplant crg_tmp.nc crg_tmp1.nc
				ncatted -O -a cell_methods,Crgplant,d,c, crg_tmp1.nc crg_tmp.nc  # Rg of grid cell
			    ncwa -O -a patch -w patchfrac -v Clabloss clabloss_tmp.nc clabloss_tmp1.nc
				ncatted -O -a cell_methods,Clabloss,d,c, clabloss_tmp1.nc clabloss_tmp.nc  # Clabloss of grid cell

				# 3) calculate fraction of growth respiration and labloss associated with roots
				cdo -O -s add calloc_wood_tmp.nc calloc_leaf_tmp.nc calloc_tmp1.nc
				cdo -O -s add calloc_tmp1.nc calloc_froot_tmp.nc calloc_total_tmp.nc
				ncwa -O -a patch -w patchfrac calloc_total_tmp.nc calloc_total_grid_tmp.nc  # grid cell-level 
				cdo -O -s mulc,${frac_wood_bg} calloc_wood_tmp.nc calloc_croot_tmp.nc       # coarse roots
				cdo -O -s add calloc_croot_tmp.nc calloc_froot_tmp.nc calloc_root_tmp.nc    # all roots
				ncwa -O -a patch -w patchfrac calloc_root_tmp.nc calloc_root_grid_tmp.nc    # all roots grid cell-level

				cdo -O -s div calloc_root_grid_tmp.nc calloc_total_grid_tmp.nc fcalloc_root_tmp1.nc  # fraction of root growth to total growth
				cdo -O -s gec,0.0 crsoil_tmp.nc landmask_tmp1.nc
				cdo -O -s subc,1.0 landmask_tmp1.nc landmask_tmp.nc
				cdo -O -s max landmask_tmp.nc fcalloc_root_tmp1.nc fcalloc_root_tmp.nc  # avoid missing values arising from div by 0
				cdo -O -s mul crg_tmp.nc fcalloc_root_tmp.nc crg_root_tmp.nc            # Rg of roots
				cdo -O -s mul clabloss_tmp.nc fcalloc_root_tmp.nc labloss_root_tmp.nc   # labile C loss of roots

				# 4) sum up all components
				cdo -O -s add crm_tmp1.nc crm_tmp3.nc crm_tmp.nc
				cdo -O -s add crsoil_tmp.nc crm_tmp.nc tmp1.nc
				cdo -O -s add crg_root_tmp.nc labloss_root_tmp.nc tmp2.nc
				cdo -O -s add tmp1.nc tmp2.nc tmp3.nc

				# 5) delete patchfrac variable and rename variable
				ncrename -O -v CallocWood,${var} tmp3.nc
				ncks -O -C -x -v patchfrac tmp3.nc tmp.nc
				ncatted -O -a cell_methods,${var},d,c, tmp.nc
				;;
			fAllocRoot)
			...
			fWoodLitter)
				...
				;;
			fRootLitter)
			    cdo -s expr,'fRootLitter=0.3*FluxCWoodtolitter+FluxCFineRoottolitter;' ${filetype}_output.nc tmp1.nc
				cdo -s vertsum tmp1.nc tmp.nc
				;;
			fLitterSoil)
			...
			nRoot)
				...
				cdo -s mulc,${frac_wood_bg} nwood_tmp1.nc nwood_tmp.nc 
        ...
        if [[ $test_suite -eq 0 ]] ; then
	   		...
	   		ncatted -O -a commit,global,c,c,"${commit}" tmp.nc
...
if [[ ("${exp}" == *"S3"*) && (-f S2/${modelname}_S2_nbp.nc || -f S2/${modelname}_S2_nbp.nc.gz) && ( "${vars}" == *"fLuc"* ) ]] ; then
   ...
   cdo -z zip_${compression} sub S2/${modelname}_S2_nbp.nc S3/${modelname}_S3_nbp.nc S3/tmp.nc
